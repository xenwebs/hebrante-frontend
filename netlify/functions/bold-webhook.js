...
export const handler = async (event) => {
  ...
  try {
    ...
    try {
      putRes = await fetchWithTimeout(`${STRAPI_URL}/api/orders/${orderRef}`, {
        ...
        body: JSON.stringify({
          data: {
            orderStatus: "paid",
            // ⚠️ Requiere el campo "shippingStatus" (enumeration: pending/shipped) en el
            //    content-type "order" de Strapi. Mientras no exista, Strapi lo ignora
            //    en silencio y no rompe nada — pero la guía NO se creará sola hasta
            //    que crees el campo y confirmes el envío manualmente desde el panel.
    ...
    // 7) La guía de Envia YA NO se crea aquí automáticamente.
    //    Ahora se genera manualmente desde el panel de envíos
    //    (/admin/envios.html) cuando el producto esté listo para despachar.
    console.log("ℹ️ Pedido pagado, pendiente de confirmación manual de envío:", orderRef)
