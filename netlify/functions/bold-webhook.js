...
exports.handler = async (event) => {
  ...
  // 2) Calcular firma con las DOS variantes posibles del algoritmo
  const base64Body = rawBytes.toString("base64")
  const sigFromBase64 = crypto.createHmac("sha256", BOLD_IDENTITY_KEY).update(base64Body).digest("hex")
  const sigFromRaw    = crypto.createHmac("sha256", BOLD_IDENTITY_KEY).update(rawBytes).digest("hex")
 
  const signature = event.headers["x-bold-signature"] || ""
 
  // --- DIAGNÓSTICO (quitar cuando funcione) ---
  console.log("🔑 Largo de la llave:", BOLD_IDENTITY_KEY.length)
  ...
  if (!isValid) {
    console.warn("⚠️ Firma inválida — ninguna variante coincide. Revisa la LLAVE DE IDENTIDAD (¿es la de producción?).")
