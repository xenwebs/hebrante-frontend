...
// 🔐 Contraseña del panel de envíos.
//    Antes esta función solo se llamaba desde bold-webhook.js (servidor → servidor).
//    Ahora también se llama desde el navegador (panel /admin/envios.html), así que
//    necesita protección: sin esta contraseña, cualquiera con la URL podría generar
//    guías reales (y hacer que se cobren) sin autorización.
//    Configúrala en Netlify → Environment variables (SHIPPING_ADMIN_PASSWORD).
...
const ORIGIN = {
  ...
  // 🏢 Info extra (edificio/torre/apto) que NO va después del "#" en `street`,
  //    para no romper el split de número. Se pega al final del street, igual
  //    que se hace con order.apartment en el destino.
  //    Ej: ENVIA_ORIGIN_COMPLEMENT="Edificio Udara Polo, Apartamento 728"
  complement: process.env.ENVIA_ORIGIN_COMPLEMENT || '',
...
export const handler = async (event) => {
  ...
  if (!SHIPPING_ADMIN_PASSWORD) {
    console.error('❌ Falta SHIPPING_ADMIN_PASSWORD en las variables de entorno')
    return {
      statusCode: 500,
      body: JSON.stringify({ error: 'SHIPPING_ADMIN_PASSWORD no configurado' })
    }
  }

  const providedPassword = event.headers['x-admin-password'] || ''
  if (providedPassword !== SHIPPING_ADMIN_PASSWORD) {
    console.warn('⚠️ Intento de crear guía con contraseña incorrecta o ausente')
    return {
      statusCode: 401,
  ...
  try {
    ...
    const origin = {
      ...
      street: [originSplit.street, ORIGIN.complement].filter(Boolean).join(' '),
      ...
      await fetch(`${STRAPI_URL}/api/orders/${orderId}`, {
        ...
        body: JSON.stringify({
          data: {
            ...
            // ⚠️ Requiere el campo "shippingStatus" en Strapi (ver bold-webhook.js).
            //    Sin este campo, Strapi lo ignora y el pedido seguirá apareciendo
            //    en el panel como pendiente aunque ya tenga trackingNumber.
