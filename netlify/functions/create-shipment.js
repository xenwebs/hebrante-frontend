...
// 🌎 Geocodes API: resuelve ciudad / código de estado a partir del código postal.
// Envia recomienda validar el CP antes de cotizar; para Colombia el estado va como
// CÓDIGO corto (no el nombre completo). Devuelve null si el CP no existe en su base.
const GEOCODES_BASE = 'https://geocodes.envia.com'

async function geocode(country, zip) {
  try {
    const res = await fetch(
      `${GEOCODES_BASE}/zipcode/${country}/${encodeURIComponent(zip)}`,
      { headers: { Authorization: `Bearer ${ENVIA_TOKEN}` } }
    )
    const json = await res.json()
    console.log(`🌎 Geocode ${country}/${zip}:`, JSON.stringify(json))

    // La respuesta es un ARRAY de coincidencias; tomamos la primera.
    const g = Array.isArray(json) ? json[0] : (json?.data || null)
    if (!g) return null

    // Código CORTO del estado (Colombia: "DC", "CUN", etc.)
    const state =
      g.state?.code?.['2digit'] ||
      g.state?.code?.['3digit'] ||
    ...
    // Colombia: city y postalCode deben ser el Código DANE (mismo valor).
    const dane = g.info?.stat || g.zip_code || zip

    return { state, city: dane, postalCode: dane }
  } catch (e) {
    console.warn(`⚠️ Error en geocode ${country}/${zip}:`, e.message)
    return null
  }
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const originGeo = await geocode('CO', ORIGIN.postalCode)
    const destGeo = await geocode('CO', String(order.zip || ''))
