...
// Deprisa rechaza el teléfono si no son 10 dígitos limpios (error "25 - TELEFONO ... INCORRECTO").
...
export const handler = async (event) => {
  ...
  try {
    ...
    if (!shipment || !shipment.trackingNumber) {
      throw new Error(genData.error?.message || JSON.stringify(genData.error) || 'Envia no devolvió número de guía')
    }
