...
export default async (req) => {
  const url = `${STRAPI}/api/banners`
    + `?sort=order:asc`
    // ВАЖНО: fields — это белый список. Всё, чего здесь нет, на фронт не приедет.
    // `order` нужен фронту для сортировки, без него порядок ломается при добавлении баннеров.
    + `&fields[0]=slug&fields[1]=title&fields[2]=order`
    + `&populate[image][fields][0]=url`
    + `&populate[image_mobile][fields][0]=url`
    + `&populate[collection][fields][0]=slug`
    // Ручная привязка конкретных продуктов к баннеру (альтернатива collection).
    ...
    + `&populate[products][fields][0]=title`
    + `&populate[products][fields][1]=title_en`
    + `&populate[products][fields][2]=slug`
    + `&populate[products][fields][3]=price`
    + `&populate[products][fields][4]=discount_percent`
    + `&populate[products][populate][images][fields][0]=url`
    + `&populate[products][populate][collection][fields][0]=slug`
    + `&populate[products][populate][collection][fields][1]=title`
