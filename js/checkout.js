...
async function handleSubmit(e, cart) {
  ...
  try {
    ...
    await processPaymentWithBold(cart, null, order.data.id)
...
async function processPaymentWithBold(cart, formData, orderId) {
  ...
  try {
    ...
    script.setAttribute("data-api-key", buttonConfig.apiKey)
    script.setAttribute("data-order-id", buttonConfig.orderId)
    script.setAttribute("data-amount", buttonConfig.amount)
    script.setAttribute("data-currency", buttonConfig.currency)
    script.setAttribute("data-integrity-signature", buttonConfig.integritySignature)
    script.setAttribute("data-redirection-url", buttonConfig.redirectionUrl)
    script.setAttribute("data-description", buttonConfig.description)

    script.onload = () => {
      console.log("✅ Bold SDK loaded")
    }
