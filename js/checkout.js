...
async function processPaymentWithBold(cart, formData, orderId) {
  ...
  try {
    ...
    const paymentUrl =
      `https://checkout.bold.co/checkout?` +
      `apiKey=${buttonConfig.apiKey}` +
      `&amount=${buttonConfig.amount}` +
      `&currency=${buttonConfig.currency}` +
      `&orderId=${buttonConfig.orderId}` +
      `&integritySignature=${buttonConfig.integritySignature}` +
      `&description=${encodeURIComponent(buttonConfig.description)}` +
      `&redirectionUrl=${encodeURIComponent(buttonConfig.redirectionUrl)}`
