...
console.log(`🔧 Bold configurado para: ${BOLD_CONFIG.MODE}`)
...
// null  → промокод действует на ВСЕ товары
// [id1, id2, ...] → промокод действует ТОЛЬКО на эти товары (Strapi product id)
let promoApplicableProductIds = null
...
// Считает сумму скидки: если promoApplicableProductIds задан — скидка
// применяется только к товарам из этого списка, иначе — ко всей корзине
function calculateDiscount(cart, percent, applicableIds) {
  if (!percent) return 0

  const eligibleItems = applicableIds
    ? cart.filter(item => applicableIds.includes(item.id))
    : cart
  ...
  return Math.round(eligibleSubtotal * (percent / 100))
}
...
function updateTotals(cart) {
  ...
  // Применить скидку (учитывая, к каким товарам применим промокод)
  const discount = calculateDiscount(cart, discountPercent, promoApplicableProductIds)
...
async function validateAndApplyPromo(code, cart) {
  try {
    ...
    const response = await fetch(`${API_URL}/api/promo-codes?filters[code][$eqi]=${code}&filters[is_active][$eq]=true&populate=applicable_products`)
    ...
    // Determinar productos aplicables (si la lista está vacía => aplica a todos)
    // Soporta tanto formato plano (Strapi v5: array directo) como formato
    // envuelto (Strapi v4/plugin config: { data: [...] })
    // NOTA: en el carrito, item.id en realidad contiene el SLUG del producto
    // (no el id numérico de Strapi), así que comparamos por slug.
    const rawApplicable = promoData.applicable_products
    const relatedProducts = Array.isArray(rawApplicable)
      ? rawApplicable
      : (rawApplicable && Array.isArray(rawApplicable.data) ? rawApplicable.data : [])
    const applicableIds = relatedProducts.length > 0
      ? relatedProducts.map(p => p.slug || p.id)
      : null

    // Si el código aplica solo a ciertos productos, verificar que al menos
    // uno de ellos esté en el carrito
    if (applicableIds) {
      const hasEligibleItem = cart.some(item => applicableIds.includes(item.id))
      if (!hasEligibleItem) {
        showPromoError("Este código no aplica a los productos de tu carrito")
        return
      }
    }
    ...
    console.log(`✅ Promoción aplicada: ${code} (${discountPercent}% descuento)`, applicableIds ? `→ solo productos: ${applicableIds.join(", ")}` : "→ todo el carrito")
...
function clearPromoCode(cart) {
  ...
  discountPercent = 0
  promoApplicableProductIds = null
...
async function processPaymentWithBold(cart, formData, orderId) {
  ...
  // Aplicar descuento (según productos aplicables)
  const discount = calculateDiscount(cart, discountPercent, promoApplicableProductIds)
  const subtotalAfterDiscount = subtotal - discount
  const total = subtotalAfterDiscount + shippingCost
  ...
  try {
    ...
    boldElement.setAttribute("data-integrity-signature", buttonConfig.integritySignature)
    boldElement.setAttribute("data-redirection-url", buttonConfig.redirectionUrl)
    boldElement.setAttribute("data-description", buttonConfig.description)
...
async function createOrder(cart) {
  try {
    ...
    // Aplicar descuento al subtotal (según productos aplicables)
    const discount = calculateDiscount(cart, discountPercent, promoApplicableProductIds)
    const subtotalAfterDiscount = subtotal - discount
    const total = subtotalAfterDiscount + shippingCost
