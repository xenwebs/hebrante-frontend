...
async function handleSubmit(e, cart) {
  ...
  try {
    ...
    console.log("✅ Pedido creado! ID:", order.data.id)
...
export async function updateOrderWithPayment(orderId, paymentData) {
  try {
    console.log("🔄 Actualizando pedido con datos de pago...", paymentData)
    
    const updateData = {
      data: {
        orderStatus: "completed",  // Cambiar статус на завершено
    ...
    if (!response.ok) {
      const errorData = await response.json()
      throw new Error(errorData.error?.message || "Error al actualizar el pedido")
    }

    const result = await response.json()
    console.log("✅ Pedido actualizado con datos de pago:", result)
