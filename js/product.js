...
import { getPriceView, getFinalPrice } from "./price.js"
import { isProductSoldOut } from "./stock.js"   // ✅ общая логика наличия (раньше жила в этом файле)

const API_URL = "https://proper-gem-a18dd78c57.strapiapp.com"

// ✅ Настройки блока рекомендаций
const RECOMMENDED_COUNT = 3          // сколько карточек показывать
const HIDE_SOLD_OUT_IN_RECOMMENDED = true  // прятать распроданные (если хватает товаров)
...
// ✅ Разбивает текст на параграфы по пустой строке, оборачивает в <p>
function formatParagraphs(text) {
  if (!text) return ""
  return text
    .split(/\n\s*\n/)          // разделитель — пустая строка между абзацами
    .map(p => p.trim())
    .filter(Boolean)
    .map(p => `<p>${p.replace(/\n/g, "<br>")}</p>`) // одиночный \n внутри абзаца -> <br>
    .join("")
...
// ✅ Проверка "товар распродан" переехала в js/stock.js —
// теперь один и тот же код используют и страница товара, и карточка каталога.
// Здесь она импортируется сверху (isProductSoldOut).

// ✅ Перемешивание массива (Фишер–Йейтс), не мутирует исходный
...
export async function loadProduct() {
  // ✅ slug вынесен ВЫШЕ try, чтобы был доступен для рекомендаций
  const params = new URLSearchParams(window.location.search)
  const slug = params.get("slug")

  // ✅ preloader и флаг ошибки — вынесены выше try, чтобы finally-блок
  // тоже имел к ним доступ (там теперь единственное место, где реально
  // прячется прелоадер — см. комментарий в finally ниже)
  const preloader = getPreloader();
  let mainContentFailed = false
  ...
  try {
    ...
    // ✅ Дополнительная задержка перед тем, как считать основной контент готовым
    await new Promise(resolve => setTimeout(resolve, 300));

    translatePage();
    updateSizeDropdownLabel();

    // ⚠️ Прелоадер здесь больше НЕ прячем. Раньше это было отдельным
    // необождённым setTimeout, который гонялся с рекомендациями в finally —
    // если рекомендации грузились быстрее, прелоадер мог пропасть раньше,
    // чем реально дорисовался сам товар. Теперь скрытие — только ниже,
    // в finally, одним местом, после того как ГАРАНТИРОВАННО готово и то,
    // и другое.
    ...
    // ✅ finally: рекомендации грузятся даже если основной товар упал / не найден
    await loadRecommendedProducts(slug)

    // ✅ Единственное место во всём loadProduct(), где прелоадер реально
    // прячется. К этому моменту завершено и основное содержимое товара
    // (или оно упало — тогда mainContentFailed = true), и блок рекомендаций.
    // Больше никакой гонки между двумя независимыми таймерами.
    preloader.hide(mainContentFailed ? 150 : 500);
  }
}
...
// currentSlug — slug товара, страницу которого мы сейчас смотрим (исключаем его)
async function loadRecommendedProducts(currentSlug) {
  ...
  try {
    ...
    // ✅ Исключаем текущий товар прямо в запросе + берём БОЛЬШУЮ выборку,
    // чтобы было из чего выбирать случайные 3.
    // pageSize=100 обязателен: без него Strapi молча отдаёт только 25.
    ...
    // ✅ Подстраховка на клиенте — на случай если фильтр Strapi не сработал
    let pool = (data.data || []).filter(item => item.slug !== currentSlug)

    // ✅ Убираем распроданные, но только если после этого останется достаточно товаров
    if (HIDE_SOLD_OUT_IN_RECOMMENDED) {
      const inStock = pool.filter(item => !isProductSoldOut(item))
      if (inStock.length >= RECOMMENDED_COUNT) {
        pool = inStock
      }
    }

    // ✅ Если рекомендовать нечего — прячем всю секцию, чтобы не висел пустой блок
    // (прелоадер здесь больше НЕ трогаем — им теперь управляет только
    // loadProduct(), см. finally-блок выше)
    if (!pool.length) {
      grid.closest(".section")?.remove()
      return
    }

    // ✅ Случайные N из отфильтрованного пула
    const products = shuffle(pool).slice(0, RECOMMENDED_COUNT).map(item => {
    ...
    // ✅ Прелоадер здесь больше не прячем — это теперь делает только
    // loadProduct() в своём finally, один раз, после того как готово всё.

  } catch (error) {
    console.error("Ошибка загрузки рекомендуемых товаров:", error)
    // ✅ ошибку просто логируем — loadProduct() всё равно спрячет прелоадер
    // в своём finally, даже если рекомендации упали
  }
}