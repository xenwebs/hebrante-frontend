...
// ✅ Получаем глобальный прелоадер (объявлен в preloader-script.js)
// Используем window.globalPreloader или создаём fallback
...
function renderProduct(product) {
  ...
  // ✅ Генерируем кнопки размеров с учётом стока
...
function initProductUI(product) {
  ...
  options.forEach(option => {
    ...
    // ✅ Если размер недоступен (disabled), не добавляем слушатель
    if (option.disabled) {
      return
    }
