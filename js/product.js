...
export async function loadProduct() {
  try {
    // ✅ Удали все старые слушатели перед перерендером
    ...
    // ✅ ВАЖНО: жди 200ms перед инициализацией UI
    ...
    setTimeout(() => {
      translatePage();
      updateSizeDropdownLabel();  // ✅ Обнови метку размера
    }, 100);
...
// ✅ НОВАЯ ФУНКЦИЯ: обновляет метку дропдауна размера
function updateSizeDropdownLabel() {
  ...
  // Если текст - это одна из опций (XS, S, M, L, XL), не трогай
  if (['XS', 'S', 'M', 'L', 'XL'].includes(sizeSelected.textContent.trim())) {
    return
  }
  
  // Иначе обнови на правильный язык
  ...
  container.innerHTML = `
    ...
    <div class="product-layout__info">
      ...
      <div class="product-sizes">
        <div class="size-dropdown">
          ...
          <div class="size-dropdown__list">
            <button class="size-dropdown__option">XS</button>
            <button class="size-dropdown__option">S</button>
            <button class="size-dropdown__option">M</button>
            <button class="size-dropdown__option">L</button>
            <button class="size-dropdown__option">XL</button>
          </div>
        </div>
      ...
      <div class="accordion">
        ...
        <div class="accordion__content">
          <span>Realizamos envíos en toda Colombia con entrega gratuita en todos los pedidos. El tiempo estimado es de 2 a 5 días hábiles, más un procesamiento previo de 24 a 48 horas. Una vez despachado, recibirás un número de seguimiento. Los tiempos pueden variar en temporadas altas y es importante ingresar correctamente la dirección para evitar retrasos o costos adicionales.</span>
        </div>
      </div>
    </div>
...
function initProductUI(product) {
  ...
  // ✅ Проверка стока
  const stock = product.stock_by_size?.[size]
  ...
  addToCart({
    ...
    size: size,  // ← Используй эту переменную
