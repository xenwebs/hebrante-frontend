...
function mapProducts(items, lang) {
  return items.map(item => {
    ...
    return {
      ...
      // ✅ остатки — нужны карточке, чтобы показать бейдж «Agotado».
      // ВАЖНО: оба поля должна отдавать Netlify-функция get-products,
      // иначе бейдж просто не появится (см. js/stock.js → hasStockData).
...
export async function renderAllProducts(){
  try {
    ...
    const res = await fetch(`/.netlify/functions/get-products?context=catalog`)
