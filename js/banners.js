 ...
 * - сетка продуктов создаётся, если у баннера есть коллекция ИЛИ конкретные продукты
 * - на мобильном используется image_mobile, если оно заполнено
 *
 * Баннер может быть привязан либо к коллекции (collection), либо к конкретным
 * продуктам (products, many-to-many). Взаимоисключение проверяется на бэкенде
 * (lifecycle hook в Strapi). Если вдруг заполнено и то и другое — приоритет
 * отдаётся products, в консоль пишется warning.
 */
...
/**
 * Возвращает массив продуктов, вручную привязанных к баннеру (или пустой массив).
 */
function getManualProducts(bannerData) {
  return Array.isArray(bannerData?.products) ? bannerData.products : []
}
...
export async function renderBanners() {
  try {
    ...
    console.log("✅ Баннеры загружены (в порядке order):",
      banners.map(b => {
        const manual = getManualProducts(b)
        const source = manual.length
          ? `products(${manual.length})`
          : (b.collection?.slug ? `collection:${b.collection.slug}` : "—")
        return `${b.order ?? "—"}: ${b.slug} [${source}]${b.image_mobile?.url ? " +mobile" : ""}`
      }))
    ...
    banners.forEach((bannerData, index) => {
      ...
      const manualProducts = getManualProducts(bannerData)

      if (manualProducts.length > 0 && bannerData.collection?.slug) {
        ...
        console.warn(
          `⚠️ Баннер ${bannerData.slug}: заполнены и collection, и products одновременно. ` +
      ...
      } else if (bannerData.collection?.slug) {
        const { section, grid } = createGridSection()
        container.append(section)
...
function updateBanner(bannerEl, bannerData, index) {
  ...
  if (manualProducts.length === 1) {
    ...
    bannerEl.onclick = () => {
      window.location.href = `/pages/product.html?slug=${manualProducts[0].slug}`
    }
  } else if (manualProducts.length === 0 && bannerData.collection?.slug) {
    bannerEl.style.cursor = "pointer"
    bannerEl.onclick = () => {
...
/**
 * Рендерит сетку продуктов, вручную выбранных для баннера в Strapi.
 * Данные уже приходят с баннером (populate в get-banners.js), поэтому
