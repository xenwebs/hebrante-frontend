...
export async function renderBanners() {
  try {
    ...
    for (const bannerData of data.data) {
      ...
      // ✅ ДОБАВЬ: Первый баннер грузится с высоким приоритетом
      if (i === 0 && bannerData.image?.url) {
        const img = banner.querySelector(".banner__image")
        if (img) {
          img.fetchPriority = "high"  // ← Высокий приоритет для первой картинки
          img.loading = "eager"
        }
      }
...
function updateBanner(bannerEl, bannerData) {
  ...
  if (img && bannerData.image?.url) {
    img.src = bannerData.image.url.replace(/\.(jpg|png|jpeg)$/i, '.webp')
    console.log(`📸 Баннер ${bannerData.slug} изображение обновлено`)
  }
