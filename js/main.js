...
import { initWhatsAppButton } from "../components/whatsapp-button.js"
...
import { renderPromoBar } from "./promo-bar.js"
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Раздел MARCA грузится быстро — не держим прелоадер стандартные 700мс.
  // history.html и workshop.html выставляют это сами в своих скриптах,
  // у shops.html своего JS нет, поэтому делаем это здесь.
  if (path.includes('shops.html') && window.globalPreloader) {
    window.globalPreloader.minShowDuration = 250
  }
  ...
  if (path === '/' || path === '/index.html') {
    // Промо-полоса и баннеры не зависят друг от друга — грузим параллельно
    await Promise.all([
      renderPromoBar(),  // ← Промо-полоса под хедером, текст/цвет из Strapi
      renderBanners()    // ← Баннеры с продуктами
    ])
  }
  ...
  // ⚠️ НЕ делаем это на страницах, которые управляют прелоадером сами:
  //   • product.html — product.js ждёт реальный fetch к Strapi + рендер товара
  //     + рекомендации и прячет прелоадер сам, один раз, в своём finally.
  //   • history.html / workshop.html — видео приходят из Strapi асинхронно,
  //     и их размеры известны только после loadedmetadata. Если прятать
  //     прелоадер отсюда, main.js успеет раньше и заголовок прыгнет вверх.
  // main.js запускается на всех этих страницах тоже (он подключён туда
  // отдельным <script>), и его автоскрытие гонялось бы с их собственным.
  const managesOwnPreloader = [
    'product.html',
    'history.html',
    'workshop.html'
  ].some(page => path.includes(page))

  if (!managesOwnPreloader) {
  ...
  return new Promise(resolve => {
    ...
    const imgs = Array.from(
      document.querySelectorAll(
        '[data-all-products] img, [data-basics-products] img, .products-grid img, .banner img, .shops__block img'
      )
    )
