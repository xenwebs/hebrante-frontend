...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // ⚠️ НЕ делаем это на product.html — там прелоадером полностью управляет
  // product.js (он ждёт реальный fetch к Strapi + рендер товара + рекомендации
  // и прячет прелоадер сам, один раз, в своём finally). main.js тоже
  // запускается на product.html (он подключён туда отдельным <script>),
  // и раньше его автоскрытие тут гонялось с product.js — иногда main.js
  // успевал спрятать прелоадер раньше, чем товар реально дорисовался.
  const managesOwnPreloader = path.includes('product.html')
