...
// Ждём чтобы убедиться что globalPreloader инициализирован
setTimeout(() => {
  if (window.globalPreloader) {
    console.log('Скрываю прелоадер через globalPreloader');
    window.globalPreloader.hide(700);
  } else {
    console.log('globalPreloader не найден, скрываю прелоадер напрямую');
