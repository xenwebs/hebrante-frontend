...
const STRAPI_URL = 'https://proper-gem-a18dd78c57.strapiapp.com'; // подставь свой, как в остальных модулях
...
async function cargarConfig() {
  try {
    const r = await fetch(
  `${STRAPI_URL}/api/popup-suscripcions?populate=*&sort=id:asc&pagination[limit]=1`
    );
    if (!r.ok) return null;
    const cfg = normalizar((await r.json())?.data?.[0]);
    if (!cfg) return null;
    return {
      ...cfg,
...
function mostrar(cfg) {
  ...
  const enviar = async () => {
    ...
    if (hayError) {
    const primero = overlay.querySelector('.is-error');
  ...
  const exito = (esNuevo, email) => {
    cont.innerHTML = '';
    cont.classList.add('hb-popup__cont--exito');   // ← вот эта
