...
class GlobalPreloader {
  constructor() {
    ...
    this.doneNotificado = false; // событие preloader:done шлём только один раз
  }
  ...
  // единая точка «прелоадер скрыт» — отсюда стартуют модули,
  // которым нельзя запускаться поверх заставки (например попап подписки)
  notificarDone() {
    if (this.doneNotificado) return;
    this.doneNotificado = true;
...
// если разметки прелоадера на странице нет, hide() выходит раньше времени
// и событие не улетит вообще — подстраховываемся после готовности DOM
document.addEventListener('DOMContentLoaded', () => {
  if (!globalPreloader.preloader) globalPreloader.notificarDone();
});
